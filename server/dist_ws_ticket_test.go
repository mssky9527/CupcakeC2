...
// go:embed embeds (server/dist) mints short-lived tickets and never opens
// pty/shell/build-logs WebSockets with durable ?token=.
//
// This drives the real shipped asset tree on disk (the same tree //go:embed dist/*
// embeds at build time). Rebuild with: powershell -File scripts/build-frontend.ps1
func TestShippedDistUsesWSTicketsNotDurableQueryToken(t *testing.T) {
	...
	if st, err := os.Stat(dist); err != nil || !st.IsDir() {
		t.Fatalf("shipped dist missing at %s — run scripts/build-frontend.ps1 before go test: %v", dist, err)
	}
